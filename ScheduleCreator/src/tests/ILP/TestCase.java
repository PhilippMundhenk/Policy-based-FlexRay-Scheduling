...
public class TestCase implements FlexRayTester
{
	...
	public TestCase(String testCaseFile, FileFormat fileFormat, ReductionMode reductionMode)
	{
		...
		try
		{
			schedule = new FlexRaySchedule(new File("").getAbsolutePath().concat("/../../examples/fibextest2.xml"), "FlexRayNetwork_1", "Channel_A_FlexRayNetwork_1", FlexRayConstants.MAX_CYCLE_NUMBER, "");
		}
		catch (Exception e)
		{
			e.printStackTrace();
		...
		try
		{
			...
			setMsgs(flexRay.FlexRayFileInput.ReadFile(new File("").getAbsolutePath().concat("/../../../Master-Thesis/Documents/schedule/paper/periods/"+testCaseFile+"_newFormat.txt"), fileFormat, reductionMode));
